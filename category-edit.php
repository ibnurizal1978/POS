...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        if ($action === 'delete') {
            // Delete category
            $stmt = $conn->prepare("
                UPDATE categories SET is_active = 0, updated_at = UTC_TIMESTAMP() 
                WHERE id = ? AND store_id = ? LIMIT 1
            ");
            ...
            if (!empty($name)) {
                ...
                $stmt = $conn->prepare("SELECT COUNT(id) FROM categories WHERE store_id = ? AND name = ? AND id != ?");
                ...
                if ($count > 0) {
                    ...
                    if ($stmt->execute([$name, $description, $id, $_SESSION['store_id']])) {
                        header("Location: categories.php");
                        exit;
                    }
                }
...
<body class="bg-gray-100">
    ...
    <div class="min-h-screen">
        ...
        <div class="max-w-7xl mx-auto px-4 py-6">
            <div class="bg-white rounded-lg shadow-sm p-6">
                ...
                <form method="POST" class="space-y-4" id="categoryForm">
                    ...
                    <div>
                        ...
                        <textarea id="description" 
                                  name="description" 
                                  rows="3"
                                  class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500"><?php echo htmlspecialchars($category['description'] ?? ''); ?></textarea>
                    </div>
    ...
    <div id="deleteModal" class="fixed inset-0 bg-black bg-opacity-50 hidden items-center justify-center z-50">
        <div class="bg-white rounded-lg p-6 m-4 max-w-sm w-full">
            <h3 class="text-lg font-medium text-gray-900 mb-4">Hapus Kategori</h3>
