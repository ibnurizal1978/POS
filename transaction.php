...
function getProductByBarcode($barcode) {
    ...
    try {
        $stmt = $conn->prepare("
            ...
            LEFT JOIN wholesale_prices wp ON wp.product_id = p.id 
                AND wp.store_id = p.store_id
                AND wp.is_active = 1
        ...
        // Debug log
        error_log("Query result for barcode $barcode: " . json_encode($product));
        
        // Format wholesale data jika ada
        if ($product && $product['wholesale_id']) {
            $product['wholesale'] = [
                'min_qty' => (int)$product['wholesale_min_qty'],
                'price' => (float)$product['wholesale_price']
            ];
            
            // Debug log
            error_log("Wholesale data formatted: " . json_encode($product['wholesale']));
        }
        
        // Hapus kolom yang tidak diperlukan
        unset($product['wholesale_id']);
        unset($product['wholesale_min_qty']);
...
function searchProducts($term) {
    ...
    try {
        ...
        // Format wholesale data untuk setiap produk
...
<body class="bg-gray-100">
    ...
    <script>
        ...
        function addItem(product) {
            console.log('Adding item:', product); // Debug log
            ...
            if (existingItem) {
                if (existingItem.quantity < product.stock) {
                    ...
                    if (product.wholesale && existingItem.quantity >= product.wholesale.min_qty) {
                        existingItem.price = parseFloat(product.wholesale.price);
                        existingItem.is_wholesale = true;
                        if (!existingItem.original_price) {
                            existingItem.original_price = parseFloat(product.selling_price);
                        }
                        showToast('Harga grosir diterapkan!', 'success');
                    }
                ...
                if (product.stock > 0) {
                    // Set initial price (might be wholesale if min_qty is 1)
                    const price = product.is_wholesale ? product.wholesale.price : product.selling_price;
                    ...
                    items.push({
                        ...
                        original_price: product.original_price || parseFloat(product.selling_price),
        ...
        function updateQuantity(index, increment) {
            ...
            console.log('Updating quantity for item:', item); // Debug log
            ...
            if (increment) {
                ...
                if (item.quantity < item.stock) {
                    ...
                    // Check if quantity meets wholesale minimum
                    if (item.wholesale && item.quantity >= item.wholesale.min_qty && !item.is_wholesale) {
                ...
                // Check if quantity no longer meets wholesale minimum
                if (item.wholesale && item.quantity < item.wholesale.min_qty && item.is_wholesale) {
                    item.price = parseFloat(item.original_price);
                    item.is_wholesale = false;
                    showToast('Harga normal diterapkan', 'success');
                }
        ...
        async function processTransaction() {
            console.log('Inside processTransaction function');
            if (items.length === 0) {
                showToast('Tidak ada item untuk diproses', 'error');
                return;
            }

            // Recheck all prices before processing
            try {
                for (let i = 0; i < items.length; i++) {
                    ...
                    const response = await fetch(`get-product.php?${params.toString()}`);
                    const product = await response.json();
                    
                    if (product && product.is_wholesale) {
                        item.price = parseFloat(product.selling_price);
                        item.is_wholesale = true;
                ...
                const result = await fetch('transaction-process', {
                    method: 'POST',
                    headers: {
        ...
        // Print options, ini redirect ke halaman print.php
        function printOptions(invoice_number) {
            window.location.href = `print.php?invoice_number=${invoice_number}`;
        }
